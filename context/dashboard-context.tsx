...
import type { DashboardContextValue } from '@/types/state';
...
export function DashboardProvider({ children }: { children: React.ReactNode }) {
  ...
  const allowedSkillIds = useMemo(() => SKILLS.map((skill: { id: string }) => skill.id), [SKILLS]);
  usePersistence(dispatch, persistable, allowedSkillIds);

  // ── Helpers ─────────────────────────────────────────────────────

  const addLog = useCallback((agent: string, msg: string, logType = '') => {
    dispatch({ type: 'ADD_LOG', agent, msg, logType });
  }, []);

  const getTaskModel = useCallback((taskId: string) => {
    return state.taskModels[taskId]
      || TASKS.find((t: { id: string; defaultModel?: string }) => t.id === taskId)?.defaultModel
      || MODELS[0]?.id;
  }, [state.taskModels, TASKS, MODELS]);

  const getTaskSkills = useCallback((taskId: string) => {
    const allowed = new Set(allowedSkillIds);
    const current = state.taskSkills[taskId] || TASKS.find((t: { id: string; defaultSkills?: string[] }) => t.id === taskId)?.defaultSkills || [];
    return current.filter((skillId: string) => allowed.has(skillId));
  }, [state.taskSkills, TASKS, allowedSkillIds]);

  // ── Skill management ────────────────────────────────────────────

  const attachSkill = useCallback((taskId: string, skillId: string) => {
    dispatch({ type: 'ATTACH_SKILL', taskId, skillId });
    addLog('SYSTEM', `Attached ${skillId} to ${taskId}`);
  }, [addLog]);

  const detachSkill = useCallback((taskId: string, skillId: string) => {
    dispatch({ type: 'DETACH_SKILL', taskId, skillId });
    addLog('SYSTEM', `Detached ${skillId} from ${taskId}`);
  }, [addLog]);

  const setTaskModel = useCallback((taskId: string, modelId: string) => {
  ...
  const runTask = useCallback(async (taskId: string) => {
    const task = TASKS.find((t: { id: string }) => t.id === taskId) as { id: string; num: number; title: string } | undefined;
    if (!task) return;
    ...
    const modelShort = (MODELS.find((m: { id: string; short?: string }) => m.id === model) || {} as { short?: string }).short || 'default';
    const skillIds = getTaskSkills(taskId);
    const skillNames = skillIds.map((sid: string) => SKILLS.find((s: { id: string; name: string }) => s.id === sid)?.name).filter(Boolean);
    ...
    try {
      ...
      addLog('SYSTEM', `Start failed S${task.num}: ${(e as Error).message}`, 'error');
    }
  ...
  const cancelTask = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      addLog('SYSTEM', `Cancel error: ${(e as Error).message}`, 'error');
    }
  }, [addLog]);
  ...
  const createPipeline = useCallback((name: string, taskIds: string[]) => {
  ...
  const updatePipeline = useCallback((id: string, updates: Partial<{ name: string; taskIds: string[] }>) => {
  ...
  const pollResults = useCallback(async () => {
    try {
      ...
      const results: ResultsMap = await res.json();
      const now = Date.now();
  ...
  const cleanAllTasks = useCallback(async () => {
    ...
    try {
      ...
      addLog('SYSTEM', `Clean failed: ${(e as Error).message}`, 'error');
    }
  }, [addLog]);
