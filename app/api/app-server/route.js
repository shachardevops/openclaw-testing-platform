...
let appProcess = null;
let appStatus = 'stopped'; // stopped | starting | running | errored
...
async function waitForPortState(port, { expectListening, timeoutMs = PORT_WAIT_TIMEOUT_MS } = {}) {
...
async function syncState(config) {
  if (appProcess && !isPidAlive(appProcess.pid)) {
    appProcess = null;
...
async function terminateTrackedProcess(config, logFile) {
...
async function terminatePortListeners(config, logFile) {
...
async function stopApp(config, logFile) {
