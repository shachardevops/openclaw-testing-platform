 ...
 */
function captureLogSnapshot(taskId, findingId) {
...
export async function GET() {
  ...
  const now = Date.now();
  let _sessions = null; // lazily loaded, cached for this request
  const getSessions = () => {
    if (_sessions === null) {
  ...
  const isSessionAlive = (runSessionKey) => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (payload?.status === 'failed' && taskIds.has(key) && payload.runSessionKey) {
        const hasStaleTimeout = (payload.findings || []).some(f => f.id === 'stale-timeout');
        if (hasStaleTimeout) {
          try {
            if (isSessionAlive(payload.runSessionKey)) {
              ...
              payload = {
                ...
                findings: (payload.findings || []).filter(f => f.id !== 'stale-timeout'),
              };
              fs.writeFileSync(fullPath, JSON.stringify(payload, null, 2));
              _cache[key] = { mtime: fs.statSync(fullPath).mtimeMs, payload };
            }
