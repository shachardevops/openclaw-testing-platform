...
export async function POST(request: NextRequest) {
  try {
    pruneTaskState();
    const { taskId, currentModel } = await request.json() as { taskId: string; currentModel?: string };
...
function readSessionHealth(sessionId: string): { lastActivityTs: number; totalLines: number; recentError: any } | null {
  try {
    ...
    const config = getProjectConfig();
    const errorPatterns: string[] = config.project.modelFallback?.errorPatterns || [];
...
function extractAllText(d: any): string {
  const parts: string[] = [];
...
function getModelFamily(modelId: string | undefined, models: any[]): string {
...
function resolveFallbackModel(currentFamily: string, rules: any[]): string | null {
  if (!rules || !currentFamily) return null;
  const rule = rules.find((r: any) => r.from === currentFamily);
  return rule?.to || null;
}

function resolveTaskSession(taskId: string): string | null {
  try {
...
function countRecentSubagents(): number {
  try {
