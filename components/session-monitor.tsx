    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden mb-6">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <h3 className="text-sm font-semibold flex items-center gap-2">
          <span>Orphaned Sessions Monitor</span>
        </h3>
        <div className="flex gap-2 items-center">
          <label className="text-[10px] text-zinc-500">
            ...
            <select
              value={maxAgeMin}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setMaxAgeMin(Number(e.target.value))}
        ...
        {sessions.length === 0 ? (
          ...
          sessions.map((s: Record<string, unknown>, i: number) => (
            <div key={i} className="py-1.5 border-b border-border last:border-b-0">
              <div className="font-semibold">{(s.key as string) || (s.sessionId as string)}</div>
              <div className="text-zinc-500">age: {s.ageMin as number}m · model: {(s.model as string) || 'n/a'} · kind: {(s.kind as string) || 'n/a'}</div>
            </div>
          ))
        )}
      </div>
