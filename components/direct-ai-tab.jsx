...
function ProviderDot({ available, label }) {
  return (
...
function StatCard({ label, value, sublabel, color = 'text-zinc-100' }) {
  return (
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {stats.cacheHits > 0 && <Legend color="bg-cyan-500" label={`Cache ${cacheRate}%`} />}
        {stats.claudeCalls > 0 && <Legend color="bg-orange-500" label={`Claude ${claudeRate}%`} />}
        {stats.codexCalls > 0 && <Legend color="bg-emerald-500" label={`Codex ${codexRate}%`} />}
        {stats.gatewayFallbacks > 0 && <Legend color="bg-purple-500" label={`Gateway ${gatewayRate}%`} />}
      </div>
    </div>
  );
}
    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="px-4 py-3 border-b border-border bg-card/20 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-3">
          ...
          <StatCard label="Cache Hits" value={stats?.cacheHits || 0} color="text-cyan-400" sublabel={stats?.totalCalls > 0 ? `${((stats.cacheHits / stats.totalCalls) * 100).toFixed(0)}% hit rate` : ''} />
