...
interface AppLogTabProps {
  onLoadingChange?: (loading: boolean) => void;
}

type LineClass = 'error' | 'warn' | 'success' | 'normal';
...
export default function AppLogTab({ onLoadingChange }: AppLogTabProps) {
  ...
  const errorCount = lines.filter((l: string) => classifyLine(l) === 'error').length;
  const warnCount = lines.filter((l: string) => classifyLine(l) === 'warn').length;
