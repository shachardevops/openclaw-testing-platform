...
function FilterBar({ filters, toggleFilter, nodeCount, edgeCount }) {
  return (
...
export default function KnowledgeGraphTab() {
  ...
  const nodesKey = useMemo(() => initialNodes.map(n => n.id).sort().join(','), [initialNodes]);
  const edgesKey = useMemo(() => initialEdges.map(e => e.id).sort().join(','), [initialEdges]);

  // Update nodes/edges when the source data changes
  // Use a ref to avoid resetting user-dragged positions unnecessarily
  ...
  if (nodesKey !== prevNodesKeyRef.current) {
    ...
    // Only reset if the set of nodes changed (not just data updates)
