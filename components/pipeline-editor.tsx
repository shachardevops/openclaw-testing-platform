...
interface PipelineEditorProps {
  pipeline: CustomPipeline | null;
  onClose: () => void;
}

export default function PipelineEditor({ pipeline, onClose }: PipelineEditorProps) {
  const { createPipeline, updatePipeline } = useDashboard();
  const { tasks: TASKS } = useProjectConfig();
  const isEdit = !!pipeline;
  const [name, setName] = useState(pipeline?.name || '');
  const [selected, setSelected] = useState<Set<string>>(new Set(pipeline?.taskIds || []));
  ...
  const save = () => {
    ...
    if (isEdit && pipeline) updatePipeline(pipeline.id, { name: name.trim(), taskIds });
