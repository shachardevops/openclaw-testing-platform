...
function getWorkspace() {
  ...
  _workspace = resolvePath(project.workspace) || BASE_DIR;
  return _workspace;
}
...
export function appLogPath() {
  const { project } = getProjectConfig();
  return resolvePath(project.targetApp?.logFile) || path.join(getWorkspace(), 'app-server.log');
}
...
export function targetAppConfig() {
  ...
  if (cfg) {
    return { ...cfg, path: resolvePath(cfg.path), logFile: resolvePath(cfg.logFile) };
  }
  return cfg;
}
...
try {
  ...
  console.warn('[config] Failed to ensure results dir:', e.message);
}
