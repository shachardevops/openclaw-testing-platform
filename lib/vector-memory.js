...
// ---------------------------------------------------------------------------
// Rust service proxy — try the Rust vector-service first, fall back to JS
// ---------------------------------------------------------------------------

let _rustClient = null;
async function _getRustClient() {
  if (_rustClient !== undefined && _rustClient !== null) return _rustClient;
  try {
    const mod = await import('./service-client');
    _rustClient = mod.vectorServiceClient || null;
  } catch {
    _rustClient = null;
  }
  return _rustClient;
...
async function _tryRustInsert(collection, id, text, metadata) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/insert', {
      method: 'POST',
      body: JSON.stringify({ id, text, collection, metadata }),
    });
  } catch { return null; }
}

async function _tryRustSearch(collection, query, limit, minSimilarity) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/search', {
      method: 'POST',
      body: JSON.stringify({ query, collection, limit, min_similarity: minSimilarity }),
    });
  } catch { return null; }
}

async function _tryRustHybridSearch(collection, query, limit) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/hybrid-search', {
      method: 'POST',
...
async function _tryRustSearchAll(query, limit) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/search-all', {
      method: 'POST',
...
class VectorCollection {
  ...
  async insert(id, text, metadata = {}) {
    ...
    // Priority 1: Rust microservice
    const rustResult = await _tryRustInsert(this.name, id, text, metadata);
    if (rustResult?.ok) return { ok: true, native: false, rust: true };
  ...
  async search(queryText, limit = 10, minSimilarity = 0.5) {
    ...
    // Priority 1: Rust microservice
    const rustResult = await _tryRustSearch(this.name, safeQuery, limit, minSimilarity);
    if (rustResult?.ok && Array.isArray(rustResult.results)) return rustResult.results;
  ...
  async hybridSearch(queryText, limit = 10) {
    // Priority 1: Rust microservice
    const rustResult = await _tryRustHybridSearch(this.name, queryText, limit);
    if (rustResult?.ok && Array.isArray(rustResult.results)) return rustResult.results;
...
class VectorMemoryManager {
  ...
  async searchAll(query, limit = 10) {
    ...
    // Priority 1: Rust microservice (cross-collection)
    const rustResult = await _tryRustSearchAll(query, limit);
    if (rustResult?.ok && Array.isArray(rustResult.results)) return rustResult.results;
