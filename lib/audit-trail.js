...
// ---------------------------------------------------------------------------
// Rust service proxy — try the Rust audit-service first, fall back to JS
// ---------------------------------------------------------------------------
...
async function _getRustClient() {
  ...
  try {
    ...
    _rustClient = mod.auditServiceClient || null;
  } catch {
    _rustClient = null;
  }
  return _rustClient;
...
async function _tryRustRecord(category, action, data, actor) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/record', {
      method: 'POST',
      body: JSON.stringify({ category, action, data, actor }),
    });
  } catch { return null; }
}

async function _tryRustQuery(params) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    ...
    return await client.fetch(`/query?${qs.toString()}`);
  } catch { return null; }
}

async function _tryRustVerifyChain() {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/verify-chain', { method: 'POST' });
  } catch { return null; }
}

async function _tryRustReplay(taskId) {
  const client = await _getRustClient();
  if (!client) return null;
  try {
    return await client.fetch('/replay', {
      method: 'POST',
...
class AuditTrail {
  ...
  _ensureInit() {
    ...
    this._loadFromDisk().catch(() => { /* best-effort */ });
  ...
  record(category, action, data = {}, actor = 'system') {
    ...
    // Fire-and-forget to Rust service (non-blocking, best-effort)
    _tryRustRecord(category, action, data, actor).catch(() => {});
  ...
  async _flushToDisk() {
    ...
    try {
      ...
      await fs.promises.writeFile(path.join(dir, 'audit-trail.json'), JSON.stringify({
        version: 1,
  ...
  stop() {
    ...
    this._flushToDisk().catch(() => { /* best-effort */ });
  }
}
...
registry.register('auditTrail', () => auditTrail);
