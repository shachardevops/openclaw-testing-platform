...
class OrchestratorEngine {
  ...
  async _evaluate() {
    ...
    for (const entry of registry.values()) {
      ...
      if (entry.status === 'stale') {
        ...
        if (age >= cfg.staleThresholdMs && entry.escalation.level < 1) {
          ...
          if (now - lastNudge >= nudgeCooldown && this._rateLimiter.canSend()) {
    ...
    for (const [taskId, info] of Object.entries(resultsIndex)) {
      ...
      if (cooldownElapsed && this._rateLimiter.canSend()) {
  ...
  _logDecision(source, conditionType, target, action, reason, message = null) {
    ...
    const CONFIDENCE_MAP = {
      manual: 1.0, deterministic: 0.9, 'memory-recall': 0.5,
      'ai-consulted': 0.4, approved: 0.8, rejected: 1.0,
    };
    let confidence = CONFIDENCE_MAP[source] || 0.5;
    // Specific action adjustments
    if (action === 'kill') confidence = Math.max(confidence, 0.9);
    else if (action === 'swap') confidence = Math.max(confidence, 0.8);
    else if (action === 'nudge') confidence = Math.min(confidence, 0.6 + 0.1);
