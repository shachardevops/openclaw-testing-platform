...
import { askWithGatewayFallback } from './direct-ai.js';
...
import taskClaims from './task-claims.js';
import tokenTracker from './token-tracker.js';
import memoryManager from './memory-tiers.js';
...
class OrchestratorEngine {
  ...
  start() {
    ...
    // Register consensus voters for critical action validation
    consensusValidator.registerVoter('orchestrator', (actionType, ctx) => {
      // Orchestrator always approves its own deterministic decisions
      return { approve: true, reason: 'deterministic-decision', confidence: 0.9 };
    });
    consensusValidator.registerVoter('drift-detector', (actionType, ctx) => {
      const status = driftDetector.getStatus();
      const taskDrift = status.recentDriftEvents.find(e => e.taskId === ctx.taskId);
      return { approve: !taskDrift, reason: taskDrift ? 'drift-detected' : 'no-drift', confidence: 0.7 };
    });
    consensusValidator.registerVoter('self-healing', (actionType, ctx) => {
      const retry = selfHealing.shouldRetryTask(ctx.taskId || 'unknown', { error: actionType });
  ...
  async _tick() {
    ...
    try {
      ...
      // Run drift detection on active tasks
      this._runDriftCheck();
    } catch (e) {
  ...
  _runDriftCheck() {
    try {
      ...
      // Update checkpoints for running tasks
      for (const taskId of activeTasks) {
        driftDetector.recordCheckpoint(taskId, resultsIndex[taskId]);
      }
    ...
    for (const entry of registry.values()) {
      ...
      if (entry.status === 'stale') {
        ...
        if (age >= cfg.killThresholdMs && entry.escalation.level < 3 && entry.escalation.swapCount > 0) {
          // Consensus check for critical kill action
          const consensusResult = consensusValidator.evaluate('kill', { taskId: entry.taskId, sessionId: entry.sessionId });
          if (consensusResult && !consensusResult.approved) {
            this._logDecision('consensus', 'stale', entry.sessionId, 'blocked',
              `Kill blocked by consensus: ${consensusResult.reason || 'quorum not reached'}`);
            continue;
          }
...
const orchestratorEngine = globalThis.__orchestratorEngine = globalThis.__orchestratorEngine || new OrchestratorEngine();
