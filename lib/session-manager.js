...
class SessionManager {
  ...
  async _doScan() {
    ...
    for (const s of sessions) {
      ...
      // Temporal smoothing: EMA health score (0.0-1.0)
      const rawScore = Math.max(0, 1 - activityAge / ((cfg.escalation.staleThresholdMs || 180000) * 2));
      const prevHealthScore = existing?.healthScore ?? 1.0;
      const healthScore = 0.3 * rawScore + 0.7 * prevHealthScore;

      // Derive status from smoothed score (only for non-controller, non-terminal)
      if (!isController && status === 'stale' && healthScore >= 0.5) {
        status = 'healthy'; // EMA smoothing prevents flicker
      }
